...
#define PIN_V 17
#define PIN_W 18
#define DE_PIN 4             // MAX485 DE
#define RE_PIN 5             // MAX485 RE
...
#define WHEEL_ANGLE 360

/*--------------共有変数------------*/
volatile int32_t angle_share = 0;
volatile int32_t magnitude_share = 0;
volatile int8_t rotateNum_share = 0;
volatile int8_t limitRot_share = 0;
spin_lock_t *lock;
/*----------------------------------*/
...
const uint32_t phase_offset = 2090041344; // -9.63deg
...
{ // {(17bit int) and (multi rotate data)} into 16bit int
    ...
    if (rotateNum == 0)
    {
        angle_conv = -32767 * (float)(360.f * ((float)angle_17bit / 131072.f) / (float)(WHEEL_ANGLE / 2));
    }
    else if (rotateNum == -1)
    {
        angle_conv = -32767 * (float)(360.f * ((float)(angle_17bit + 65535 + 65535) / 131072.f) / (float)(WHEEL_ANGLE / 2));
    }
    else if (rotateNum == 1)
    {
        angle_conv = -32767 * (float)(360.f * ((float)(angle_17bit - 65535 - 65535) / 131072.f) / (float)(WHEEL_ANGLE / 2));
...
void core1_main()
{
    ...
    float Kd = 0.0f, alpha = 0.05f, beta = 0.9f;
    ...
    while (true)
    {
        ...
        torque = -((float)delta_angle / 65536.f * Kd) + (0.12f * (float)local_magnitude / 10000.f);
...
int main()
{
    ...
    while (true)
    {
        ...
        sleep_ms(1);       // 約1000Hzで送信
        gpio_put(PICO_DEFAULT_LED_PIN, ffb_active);
        gpio_put(24, ffb_magnitude != 0);
        if (!ffb_active)
        {
            ffb_magnitude = 0;
        }
